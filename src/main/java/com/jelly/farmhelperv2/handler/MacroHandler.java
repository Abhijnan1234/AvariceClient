...
import com.jelly.farmhelperv2.feature.impl.FishingMacro;
...
public class MacroHandler {
    ...
    //fish
    // Fishing macro instance
    private final FishingMacro fishingMacro = new FishingMacro();

    // Toggle state
    private boolean fishingMacroToggled = false;
   @Getter @Setter
    private boolean fishingStartingUp = false;
    @Getter @Setter
    private boolean fishingKeyPrev = false;
    // Runnable to start the fishing macro
    private final Runnable startFishingMacro = () -> {
        KeyBindUtils.stopMovement();      // stops player movement
        fishingMacro.onEnable();          // reset state machine
        fishingStartingUp = false;
    };
...
// Replace the existing fishing key handling in `onTick` with this:
        boolean fishingKeyActive = FarmHelperConfig.fishingKeybind.isActive();
        if (fishingKeyActive && !fishingKeyPrev) {
            // key was just pressed (edge)
            fishingMacroToggled = !fishingMacroToggled;

            if (fishingMacroToggled) {
                fishingStartingUp = true;
                Multithreading.schedule(startFishingMacro, 50, TimeUnit.MILLISECONDS);
            } else {
                fishingMacro.onDisable();
            }

            LogUtils.sendSuccess("Fishing Macro " + (fishingMacroToggled ? "Enabled" : "Disabled"));
        }
        fishingKeyPrev = fishingKeyActive;

// then continue ticking the fishing macro as before
        if (fishingMacroToggled) {
            fishingMacro.onTick();  // runs state machine each client tick
            return; // skip all other macros while fishing
        }
